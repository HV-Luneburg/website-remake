...
339. **Typografie-System (Fluid Scale)** — Die Seite nutzt eine stufenlose (fluid) CSS `clamp()` Skalierung für alle Textgrößen in `style.css` (Variablen `--text-xs` bis `--text-hero`). Tailwind's `tailwind.config.js` ist so konfiguriert, dass Standardklassen (`text-sm`, `text-lg` etc.) auf diese CSS-Variablen zugreifen.
340. **Mobile Menu Cascading Animations** — In `Header.astro` muss das AstroWind JS Toggle-Verhalten (`bg-page`) für das Mobile Menü durch harte CSS overrides (`:global(header#header.expanded)`) überschrieben werden. Das Menü füllt den gesamten Bildschirm unterhalb des Headers. Listen-Elemente fliegen durch `fadeInUpMobile` Animationen via CSS `:nth-child` Verzögerungen sanft von unten ein.
341. **Kein Overflow-X bei Buttons** — Die Klasse `.btn` (in `tailwind.css`) nutzt konsequent `white-space: normal`, da `nowrap` auf schmalen Smartphones bei langen Texten unweigerlich horizontales Scrollen auslöst.
342. **Sponsoren/Partner Grid Redesign** — Alte dunkelblaue (`slate-800/900`) Dark-Mode Tailwind-Klassen wurden verbannt. Im Dark Mode nutzt die Seite ausschließlich den Corporate-Look: Hintergrund `#141414`, Container/Karten `#1C1C1C`, Ränder `#2A2A2A`, gemischt mit Rot-Gold Akkzenten. Sponsoren-Logos erhalten oft `grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100` für einen hochwertigen Look.
343. **Vercel Adapter (Static Mode)** — Der `@astrojs/vercel` Adapter wird auf Nutzerwunsch für das Vercel Hosting verwendet, jedoch operiert die Seite weiterhin im Modus `output: 'static'`. Das Sanity CMS wird extern (direkt über Sanity) gehostet, weshalb kein `studioBasePath` und auch kein SSR (`output: 'server'`) in der Astro-Instanz benötigt wird.
