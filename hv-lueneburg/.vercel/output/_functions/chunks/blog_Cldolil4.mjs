...
import { A as AstroError, U as UnknownContentCollectionError, R as RenderUndefinedEntryError, c as createComponent, u as unescapeHTML, a as renderTemplate, i as renderUniqueStylesheet, j as renderScriptElement, k as createHeadAndContent, r as renderComponent } from './astro/server_DIIrBVhZ.mjs';
import 'piccolore';
import * as devalue from 'devalue';
import { e as cleanSlug, P as POST_PERMALINK_PATTERN, t as trimSlash, A as APP_BLOG, C as CATEGORY_BASE, T as TAG_BASE, B as BLOG_BASE } from './permalinks_plztHYTi.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_-X4xYV7-.mjs');
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_DG2KGQ_7.mjs').then(n => n._);
...
const normalizeFromSanity = async (beitrag) => {
  const { portableTextToHtml, urlFor } = await import('./sanity_CNwnMWJn.mjs');
  const slug = beitrag.slug.current;
  const publishDate = new Date(beitrag.veroeffentlichtAm);
  const category = beitrag.kategorie ? { slug: cleanSlug(beitrag.kategorie), title: beitrag.kategorie } : void 0;
  ...
  const image = beitrag.titelbild ? urlFor(beitrag.titelbild).width(1200).url() : void 0;
  const content = beitrag.inhalt ? portableTextToHtml(beitrag.inhalt) : void 0;
  return {
    id: beitrag._id,
    slug,
    permalink,
...
const load = async function() {
  try {
    const { fetchBeitraege } = await import('./sanity_CNwnMWJn.mjs');
    const beitraege = await fetchBeitraege();
    if (beitraege && beitraege.length > 0) {
      const normalizedPosts2 = beitraege.map(async (b) => await normalizeFromSanity(b));
      return await Promise.all(normalizedPosts2);
    }
  } catch {
  }
