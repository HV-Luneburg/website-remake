...
343. **Vercel Adapter Override** — Aufgrund eines Nutzer-Wunsches wird doch der `@astrojs/vercel` Adapter statt eines reinen Static-Builds verwendet. Die `astro.config.ts` ist nun auf `output: 'server'` konfiguriert, um SSR-Unterstützung (speziell für das eingebaute Sanity Studio unter `/admin`) zu gewährleisten. `netlify.toml` wurde entfernt.
